    ...
    for c_sample in range(s):
        ...
        while failed_iterations <= 10:
            try:
                ...
                ws["PN_kt"] = impartial.weighted_peer_nomination(score_matrix, k, impartial.kt_weights, eps_corrected)
                ws["PN_spearman"] = impartial.weighted_peer_nomination(score_matrix, k, impartial.spearman_weights, eps_corrected)
                ws["PN_rankstep"] = impartial.weighted_peer_nomination(score_matrix, k, impartial.rankstep_weights, eps_corrected)


                for x in [Impartial.NOMINATION, Impartial.EXACT, "EDP_rank", "PN_dist", "PN_maj", "PN_step", "PN_kt", "PN_spearman", "PN_rankstep"]:
                    key = (n, k, m, l, p, s, x)
...
if __name__ == '__main__':
    ...
    df.to_pickle("test_results_ranked_adv.pkl")
    df_sizes.to_pickle("test_sizes_ranked_adv.pkl")